...
#    len_menu= len(strings[i])
    if row_num == 0:
        ...
        if (i == position):
            box.addstr( i, 32-len(strings[i-1])//2,strings[ i - 1 ], highlightText )
        else:
            box.addstr( i, 32-len(strings[i-1])//2,strings[ i - 1 ], normalText )
        if i == row_num:
            break
    ...
    for i in range( 1 + ( max_row * ( page - 1 ) ), max_row + 1 + ( max_row * ( page - 1 ) ) ):
        if row_num == 0:
            ...
            if ( i + ( max_row * ( page - 1 ) ) == position + ( max_row * ( page - 1 ) ) ):
                box.addstr( i - ( max_row * ( page - 1 ) ), 32-len(strings[i-1])//2, strings[ i - 1 ], highlightText )
            else:
                box.addstr( i - ( max_row * ( page - 1 ) ), 32-len(strings[i-1])//2, strings[ i - 1 ], normalText )
            if i == row_num:
                break
